...
用法: `python3 <脚本路径> tools/call <工具名> ['<JSON参数>'] [--file <文件路径>] [--auto-clean]`

新增参数：
- `--file <文件路径>`：从本地文件读取内容，自动处理JSON转义，适合中文/长内容写入
- `--auto-clean`：操作完成后自动删除`--file`指定的临时文件
...
- **中文/长内容写入（推荐，自动转义）**:
  ```bash
  # 写入内容并且自动删除临时文件
  echo '## 标题\n中文内容...' > /tmp/craft_content.md
  python3 ~/.deepagents/agent/skills/craft-mcp-skill/scripts/craft_client.py tools/call markdown_add '{"pageId": "文档ID", "position": "end"}' --file /tmp/craft_content.md --auto-clean
  ```

#### 5. 修改已有 Block

- **按 block ID 修改内容**（需先通过 `blocks_get` 获取 block ID）:
  ```bash
  python3 ~/.deepagents/agent/skills/craft-mcp-skill/scripts/craft_client.py tools/call blocks_update '{"blocks": [{"id": "blockID", "markdown": "修改后的内容"}]}'
  ```
- **批量修改多个 block**:
  ```bash
  python3 ~/.deepagents/agent/skills/craft-mcp-skill/scripts/craft_client.py tools/call blocks_update '{"blocks": [{"id": "blockID1", "markdown": "内容1"}, {"id": "blockID2", "markdown": "内容2"}]}'
  ```
- **注意**: `blocks_update` 响应较慢（可能需要 15-30 秒），建议设置较长超时。获取 block ID 的方法：`blocks_get` 使用 `format="json"` 返回每个 block 的 `id` 字段。
